...
@export var min_distance_to_keep_target_after_not_visible = 15
...
func schedule_check(timeout):
	while true:
		...
		if not agent.target_list.has(current_target) or agent.target_list[current_target].visible == false:
			if agent.global_position.distance_to(current_target.global_position) > min_distance_to_keep_target_after_not_visible:
				reset_current_target(agent.target_list, current_target)
...
func on_target_list_changed(target_list: Dictionary[Node3D, BaseAgent.TargetInfo]):
	...
	if not target_list.has(current_target) or target_list[current_target].visible == false:
		var distance = agent.global_position.distance_to(current_target.global_position)
		if distance < min_distance_to_keep_target_after_not_visible:
			return
		reset_current_target(target_list, current_target)
		return
		
		

func reset_current_target(target_list, current_target):
